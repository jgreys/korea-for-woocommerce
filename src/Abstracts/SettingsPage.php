...
abstract class SettingsPage {
    ...
    public function __construct() {
        add_filter( 'woocommerce_koreakit_settings_tabs_array', array( $this, 'add_settings_page' ), 20 );
        add_action( 'woocommerce_koreakit_sections_' . $this->id, array( $this, 'output_sections' ) );
        add_action( 'woocommerce_koreakit_settings_' . $this->id, array( $this, 'output' ) );
        add_action( 'woocommerce_koreakit_settings_save_' . $this->id, array( $this, 'save' ) );
    }
    ...
    final public function get_settings_for_section( $section_id ) {
        ...
        return apply_filters( 'woocommerce_koreakit_get_settings_' . $this->id, $settings, $section_id );
    }
    ...
    public function get_sections() {
        ...
        return apply_filters( 'woocommerce_koreakit_get_sections_' . $this->id, $sections );
    }
    ...
    public function output_sections() {
        ...
        foreach ( $sections as $id => $label ) {
            $url       = admin_url( 'admin.php?page=wc-koreakit-settings&tab=' . $this->id . '&section=' . sanitize_title( $id ) );
    ...
    /**
     * Trigger the 'woocommerce_koreakit_update_options_'.id action.
     *
    ...
    protected function do_update_options_action( $section_id = null ) {
        ...
        if ( $section_id ) {
            do_action( 'woocommerce_koreakit_update_options_' . $this->id . '_' . $section_id );
        }
    }
}
